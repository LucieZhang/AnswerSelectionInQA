...
class DataServer(object):
    ...
    def __init__(self, data, vocab, opt, is_train=False, shuffle=False, use_buckets=True, volatile=False):
        self.data = data
        self.vocab = vocab
        self.opt = opt
        ...
        # Obtain bucket data
        if self.use_buckets:
            self.buckets, self.data = perform_bucketing(self.opt, self.data, is_train=is_train)
            self.bucket_id = 0
    ...
    def sent_to_idx(self, sent):
        ...
        idx_list = [self.vocab.word_to_index[word] if word in self.vocab.word_to_count.keys() and
                    self.vocab.word_to_count[word] >= self.opt.freq_bound else 1 for word in sent.split()]
        ...
        if self.bucket_id < self.opt.num_buckets:
            ...
            while len(s1_batch) < self.batch_size:
                try:
                    ...
                    if self.is_train:
                        s3 = self.sent_to_idx(self.data[self.bucket_id][0][self.pair_id][2])
        ...
        if self.pair_id < self.get_length():
            while len(s1_batch) < self.batch_size:
                try:
                    ...
                    if self.is_train:
                        s3 = self.sent_to_idx(self.data[0][self.pair_id][2])
